...
class FeatureLoss(nn.Module):
    ...
    def forward(self,
        ...
        """
        # print(preds_S.shape[-2:])
        # print(preds_T.shape[-2:])
        ...
        # loss = fg_loss + 0.1*bg_loss \
        #        +  mask_loss +  rela_loss
               
        # print('fg_loss',fg_loss)
        # print('bg_loss',bg_loss)
        # print('mask_loss',mask_loss)
        # print('rela_loss',rela_loss)
        return loss
    ...
    def get_fea_loss(self, preds_S, preds_T, Mask_fg, Mask_bg, C_s, C_t, S_s, S_t):
        ...
        # loss_coine = nn.CosineSimilarity(dim=-1, eps=1e-6)
        ...
        # fg_fea_s = fg_fea_s.view(fg_fea_s.shape[0],fg_fea_s.shape[1],-1)
        # fg_fea_t = fg_fea_t.view(fg_fea_t.shape[0],fg_fea_t.shape[1],-1)
        # bg_fea_s = bg_fea_s.view(bg_fea_s.shape[0],bg_fea_s.shape[1],-1)
        # bg_fea_t = bg_fea_t.view(bg_fea_t.shape[0],bg_fea_t.shape[1],-1)
        
        # fg_loss = torch.mean(torch.abs(loss_coine(fg_fea_s,fg_fea_t)))
        # bg_loss = torch.mean(torch.abs(loss_coine(bg_fea_s,bg_fea_t)))
    ...
    def get_mask_loss(self, C_s, C_t, S_s, S_t):
        ...
        S_s = S_s.view(S_s.shape[0],-1)
        S_t = S_t.view(S_t.shape[0],-1)
        
        # mask_loss = torch.mean(torch.abs(loss_coine(C_s,C_t))) + torch.mean(torch.abs(loss_coine(S_s,S_t)))
    ...
    def get_rela_loss(self, preds_S, preds_T):
        ...
        # out_s = out_s.view(out_s.shape[0],out_s.shape[1],-1)
        # out_t = out_t.view(out_t.shape[0],out_t.shape[1],-1)
        
        # rela_loss = torch.mean(torch.abs(loss_coine(out_s,out_t)))
